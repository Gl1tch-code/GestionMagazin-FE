...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (currectUserInfos !== undefined) {
    ...
    if (parsedCurrectUserInfos?.role === window.constants.USERS_ROLES.admin) {
      ...
      parsedCurrectUserInfos?.role === window.constants.USERS_ROLES.user
    ) {
      const query = `
                query {
      ...
      fetch(window.constants.backend_url, {
        ...
        .then((data) => {
          ...
          localStorage.setItem(
            "user-infos",
            JSON.stringify(data?.data?.utilisateurLogin)
          );
          window.location.pathname = window.location.pathname.replace(
            "/login.html",
            "/Articles.html"
        ...
        .catch(() => {
          ...
          document.querySelector(".login-error-msg").style.display =
            "inline-block";
    ...
    async function userLogin() {
      fetch(window.constants.backend_url, {
        ...
        .then((data) => {
          ...
          localStorage.setItem(
            "user-infos",
            JSON.stringify(data?.data?.adminLogin)
          );
          window.location.pathname = window.location.pathname.replace(
            "/login.html",
            "/dashboard.html"
        ...
        .catch(() => {
          ...
          document.querySelector(".admin-login-error-msg").style.display =
            "inline-block";
